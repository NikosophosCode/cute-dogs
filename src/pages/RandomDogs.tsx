...
export const RandomDogs: React.FC<RandomDogsProps> = ({
  ...
  const fetchRandomFact = async () => {
    ...
    setFactError(null);
    try {
      const randomFact = await dogAPI.getRandomDogFact();
      setFact(randomFact);
    } catch (err) {
      setFactError('No se pudo cargar el dato. Intenta de nuevo.');
      console.error(err);
    } finally {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            {/* Error Message */}
            {error && (
              <div className="bg-red-500/80 text-white rounded-lg p-4 w-full animate-fadeInScale">
            ...
            <button
              ...
              disabled={loading}
              className="group relative px-8 py-4 bg-white text-purple-600 font-bold text-lg rounded-full shadow-2xl hover:shadow-purple-500/50 transform hover:scale-110 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed animate-shake w-full"
            >
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            <div className="w-full">
              {fact ? (
                <div className="glass-effect rounded-3xl p-8 text-center animate-fadeInScale">
                ...
                <div className="glass-effect rounded-3xl p-12 text-center">
                  <div className="text-8xl mb-4 animate-float">📚</div>
                  <p className="text-white text-xl font-medium">
                    Haz click en el botón para descubrir un dato curioso
                  </p>
                </div>
              )}
            </div>
            ...
            {factError && (
              <div className="bg-red-500/80 text-white rounded-lg p-4 w-full animate-fadeInScale">
                <p className="font-semibold">❌ {factError}</p>
              </div>
            )}
            ...
            >
              <span className="relative z-10 flex items-center justify-center gap-2">
                ...
                {factLoading ? 'Cargando...' : 'Obtener Dato Curioso'}
              </span>
              <div className="absolute inset-0 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            </button>
          </div>
